...
func HandleAppleWebhook(c *gin.Context) {
	...
	if rawSig == "" {
		utils.Error(c, http.StatusBadRequest, "Missing X-Apple-Signature header")
		return
	}
	...
	if !strings.HasPrefix(rawSig, prefix) {
		utils.Error(c, http.StatusBadRequest, "Invalid signature format")
		return
	}
	...
	if secret == "" {
		utils.Error(c, http.StatusInternalServerError, "Missing APPLE_WEBHOOK_SECRET")
		return
	}
